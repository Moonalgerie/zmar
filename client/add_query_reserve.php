<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$checkin = $_POST['date'];
		$conn->query("INSERT INTO `guest` (firstname, middlename, address, contactno) VALUES('$firstname', '$middlename', '$address', '$contactno')") or die(mysqli_error());
		$query = $conn->query("SELECT * FROM `guest` WHERE `firstname` = '$firstname'&& `contactno` = '$contactno'") or die(mysqli_error());
		$fetch = $query->fetch_array();
		$query2 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `status` = 'Pending'") or die(mysqli_error());
		$row = $query2->num_rows;
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
								<label style = 'color:#ff0000;'>Date non disponible déja prise </label><br />";
